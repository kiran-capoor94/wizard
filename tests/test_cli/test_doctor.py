...
def test_db_is_healthy_returns_false_when_file_missing(tmp_path):
    from wizard.cli.doctor import db_is_healthy
    assert db_is_healthy(tmp_path / "no_such.db") is False
...
def test_db_is_healthy_returns_false_when_tables_missing(tmp_path):
    ...
    conn.commit()
    conn.close()
    assert db_is_healthy(db) is False
...
def test_db_is_healthy_returns_true_when_all_tables_present(tmp_path):
    ...
    conn.commit()
    conn.close()
    assert db_is_healthy(db) is True
