...
`WriteBackService.append_task_outcome(task, summary)` is a new method — a single Notion paragraph block append. It mirrors the existing `push_task_to_notion` pattern. If Notion is not configured, returns a `WriteBackStatus(ok=False, error="...")` and the tool continues — never blocks.
...
- `src/wizard/services.py` — promote three private sync methods to public
- `src/wizard/integrations.py` — add `NotionClient.append_task_outcome` (one method)
